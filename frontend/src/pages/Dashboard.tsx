...
const formatBookingTime = (utcTimeString: string, formatStr: string = "HH:mm") => {
  var pakistanTime = formatInTimeZone(new Date(utcTimeString + "Z"), PAKISTAN_TIMEZONE, formatStr);
  return pakistanTime;
};
...
const Dashboard: React.FC = () => {
  ...
  const fetchSelectedDateBookings = async () => {
    ...
    const listViewBookings = resp.data;
  ...
  const fetchDailySummary = async () => {
    ...
    const resp = await api.get(`/reports/report?start_date=${today}&end_date=${today}`);
    // Ensure only paid bookings are counted in revenue
    const paidBookings = resp.data.bookings?.filter((b: Booking) => b.paid) || [];
    const paidRevenue = paidBookings.reduce((sum: number, b: Booking) => sum + b.price, 0);
    setDailyReport({
      ...resp.data,
  ...
  const fetchWeeklySummary = async () => {
    ...
    const start = format(addDays(pakistanNow, -6), "yyyy-MM-dd");
    const end = format(pakistanNow, "yyyy-MM-dd");
    const resp = await api.get(`/reports/report?start_date=${start}&end_date=${end}`);
    // Ensure only paid bookings are counted in revenue
    const paidBookings = resp.data.bookings?.filter((b: Booking) => b.paid) || [];
    const paidRevenue = paidBookings.reduce((sum: number, b: Booking) => sum + b.price, 0);
    setWeeklyReport({
...
const generatePDF = (reportData: ReportData) => {
  ...
  doc.setFontSize(12);
  doc.text(`Period: ${format(new Date(reportData.start_date), "MMM d, yyyy")} - ${format(new Date(reportData.end_date), "MMM d, yyyy")}`, 20, 35);

  doc.text(`Total Bookings: ${reportData.bookings_count}`, 20, 50);
  doc.text(`Total Revenue (Paid): Rs ${reportData.total_revenue.toFixed(2)}`, 20, 60);
  ...
  autoTable(doc, {
    head: [['Date', 'Time', 'Price', 'Status']],
    body: bookingsData,
    startY: (doc.lastAutoTable?.finalY ?? 75) + 15,
    theme: 'grid'
  });

  doc.save(`court-report-${reportData.start_date}-to-${reportData.end_date}.pdf`);
};
    ...
    <div className="bg-white rounded-xl shadow p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={goToPreviousDay} className="px-3 py-2 bg-gray-200 rounded hover:bg-gray-300">
            ← Previous
          </button>
          <button onClick={goToToday} className="px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
            Today
          </button>
          <button onClick={goToNextDay} className="px-3 py-2 bg-gray-200 rounded hover:bg-gray-300">
            Next →
          </button>
          <input
            type="date"
      ...
      <div className="relative max-h-[800px] overflow-y-auto border rounded-lg">
        ...
        <div className="grid grid-rows-24 border-r">
          {hours.map(hour => (
            <div key={hour} className="h-20 border-t relative text-sm text-gray-600 pl-2">
              {hour === 0 ? "12:00 AM" :
               hour < 12 ? `${hour}:00 AM` :
               hour === 12 ? "12:00 PM" :
               `${hour - 12}:00 PM`}
    ...
    return (
      <div
        ...
        className={`p-3 rounded-lg border-l-4 ${
          booking.court_id === 1 ? 'bg-blue-50 border-blue-400' : 'bg-green-50 border-green-400'
        } hover:shadow-md transition-shadow cursor-pointer`}
      >
        <div className="font-semibold text-gray-800">{booking.customer_name}</div>
        <div className="text-sm text-gray-600">
          Court {booking.court_id} • {formatBookingTime(booking.start_time)} - {formatBookingTime(booking.end_time)}
        </div>
        <div className="text-sm font-medium text-gray-700">
          Rs {booking.price.toFixed(2)} • 
          <span className={`ml-1 ${booking.paid ? 'text-green-600' : 'text-red-600'}`}>
            {booking.paid ? 'Paid' : 'Unpaid'}
          </span>
        </div>
                    ...
                    <tr key={booking.id} className="border-b hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-3">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          booking.court_id === 1 ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                        }`}>
                          Court {booking.court_id}
                        </span>
                      </td>
                      <td className="p-3">{formatBookingTime(booking.start_time)}</td>
                      <td className="p-3">{formatBookingTime(booking.end_time)}</td>
                      <td className="p-3 font-medium">Rs {booking.price.toFixed(2)}</td>
                      <td className="p-3">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          ...
          <div className="bg-white p-6 rounded-lg w-96 shadow-lg">
            ...
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-600 mb-4">
                <strong>Time:</strong> {formatBookingTime(selectedBooking.start_time)} - {formatBookingTime(selectedBooking.end_time)}
              </p>
            </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div>
                    ...
                    <p className="text-xl font-bold text-green-600">Rs {report.total_revenue.toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Total Bookings</p>
                    <p className="text-xl font-bold text-blue-600">{report.bookings_count}</p>
                  </div>
                  <div>
                ...
                <h3 className="font-semibold mt-4 mb-2">Daily Summary:</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {Object.entries(report.per_day).map(([day, stats]) => (
                    <div key={day} className="bg-white p-3 rounded border">
                      <p className="font-medium text-gray-700">{format(new Date(day), "MMM d, yyyy")}</p>
                      <p className="text-sm text-gray-600">{stats.count} bookings - Rs {stats.revenue.toFixed(2)}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    <div className="bg-blue-50 p-4 rounded">
                      <p className="text-sm text-gray-600">Total Bookings</p>
                      <p className="text-2xl font-bold text-blue-600">{report.bookings_count}</p>
                    </div>
                    <div className="bg-green-50 p-4 rounded">
                      <p className="text-sm text-gray-600">Total Revenue (Paid)</p>
                      <p className="text-2xl font-bold text-green-600">Rs {report.total_revenue.toFixed(2)}</p>
                    </div>
                    ...
                    <table className="w-full border-collapse border border-gray-300">
                      <thead>
                        <tr className="bg-gray-100">
                          ...
                          <th className="border border-gray-300 p-2 text-left">Bookings</th>
                      ...
                      <tbody>
                        {Object.entries(report.per_day).map(([date, stats]) => (
                          <tr key={date}>
                            <td className="border border-gray-300 p-2">{format(new Date(date), "MMM d, yyyy")}</td>
                            <td className="border border-gray-300 p-2">{stats.count}</td>
                            <td className="border border-gray-300 p-2">Rs {stats.revenue.toFixed(2)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    ...
                    <table className="w-full border-collapse border border-gray-300">
                      ...
                      <tbody>
                        {report.bookings.map((booking) => (
                          <tr key={booking.id}>
                            <td className="border border-gray-300 p-2">{format(new Date(booking.start_time + 'Z'), "MMM d, yyyy")}</td>
                            <td className="border border-gray-300 p-2">
                              {formatBookingTime(booking.start_time)} - {formatBookingTime(booking.end_time)}
                            </td>
                            <td className="border border-gray-300 p-2">Rs {booking.price.toFixed(2)}</td>
                            <td className="border border-gray-300 p-2">
